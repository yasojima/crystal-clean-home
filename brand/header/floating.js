(()=>{const l=document.createElement('link');l.rel='stylesheet';l.href='/crystal-clean-home/brand/header/bottom-bar.css?v=single-product-cart1';document.head.append(l)})();
(() => {
  'use strict';
...
(() => {
  ...
  let cartIndex;
  const cartReady=(async()=>{if(!window.CCHCart)await new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='/crystal-clean-home/brand/shop/cart-core.js';script.onload=resolve;script.onerror=reject;document.head.append(script)});const inlineCatalog=document.getElementById('shop-catalog');if(inlineCatalog)window.CCHReferenceCatalog=JSON.parse(inlineCatalog.textContent);if(!window.CCHReferenceCatalog)await new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='/crystal-clean-home/reference/catalog.js?v=quantity2';script.onload=resolve;script.onerror=reject;document.head.append(script)});cartIndex=CCHCart.index(window.CCHReferenceCatalog)})();
  async function updateCart(){try{await cartReady;const cart=CCHCart.clean(JSON.parse(localStorage.getItem('cch-estimate-cart-v1')||'[]'),cartIndex),totals=CCHCart.totals(cart,cartIndex);const count=cart.reduce((n,l)=>n+l.qty,0),badge=cartButton.querySelector('.cch-bottom-cart-count');badge.textContent=count;badge.hidden=count===0;cartButton.setAttribute('aria-label','お見積り概要を表示する（'+count+'点）');popup.querySelector('[data-count]').textContent=count;popup.querySelector('[data-amount]').textContent=totals.total.toLocaleString()+(totals.quote?'＋個別見積り':'')}catch(error){console.error('Cart summary:',error);popup.querySelector('[data-amount]').textContent='—'}}
