...
法人向けページ `/service/corporation/` は、商品ページと共通の紺色・游明朝の全幅見出し帯、独自の本文7箇所、人物を含まないリアルなアニメ調の画像18点、白・淡い水色・紺の端末別CSSを使用する。正本は `corporation.py`、`brand/corporation/images/`、`source/device/{desktop,mobile}/css/corporation.css`。旧ラフ出力 `docs/_product-wireframe/` は現行ビルドで生成しない。
