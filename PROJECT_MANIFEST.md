...
「初めての方へ」 `/first/` に3項目を統合した漫画LPを制作。承認済み `source/first/lp-copy.md` を要約し、吹き出し付き4コマ、清掃への取り組み、感染予防、匿名A社〜E社との比較サンプル、ご利用の流れ、FAQ、既存型の見積CTAを配置する。人物は冒頭画像の2人に統一し、リアルなアニメ調ではなく参考イラストに近い説明漫画調とする。本文と吹き出しは表示される静的HTML。基調色は紺・水色・白、CTAは赤/白反転。PC4列・中幅2列・スマホ1列で漫画を読む。比較は自社を含む仮設定と明記する。既存の住まい画像、比較イメージ2組、掲載用サンプルの声3件、商品8カテゴリへの導線を加える。衛生管理と末尾のあいさつも同じ人物の漫画で説明し、見積CTAは既存の共有型を5箇所に配置する。比較画像と口コミは実績ではない旨を明示する。素材の選択はbrand/first-lp/materials.jsonで管理し、ホームのカテゴリ名称・リンク・画像と商品の口コミ正本を再利用する。正本と生成仕様は `FIRST_LP.md`。3アンカーと4項目ナビを維持し、独立 `/reason/` は復活させない。研修ページは維持する。
