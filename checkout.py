...
def publish_checkout(root,header,data):
    ...
    for name,soup,path in [('cart',cart,'/cart/'),('estimate',estimate,'/estimate/')]:
        ...
        if name=='estimate':shell=shell.replace('<script defer src="'+BASE+'reference/checkout.js?v=empty-source2">','<script defer src="'+BASE+'reference/postal-data.js"></script><script defer src="'+BASE+'reference/checkout.js?v=empty-source2">')
